...
class DownArticle:
    ...
    def get_article_list(self, url, page_number):
        ...
        if data is not None:
            ...
            self.save_jkes(self.file_path.format(page_number), articles)
                #my_print.my_print(href)

    def run(self, url_queue):
        '''
        get queue
        get article_list
        '''
        while url_queue.empty() is not True:
            url_pg = url_queue.get()
            url = self.url.format(url_pg)
            self.get_article_list(url, url_pg)
            url_queue.task_done()
...
if __name__ == "__main__":
    ...
    #url = 'https://www.vipyl.com/article/143/list_{}.html'
    #url = 'https://www.vipyl.com/article/143/418218.html'
    u = 'https://www.vipyl.com/article/143/410747.html' #410747
    url = 'https://www.vipyl.com/article/329/list_{}.html'

    #file_path = RESULT_DIR + '经典搞笑语录_{}.txt'
    file_path = RESULT_DIR + '名人名言_{}.txt'
    
    article = DownArticle(file_path, url)
    article.create_dir(RESULT_DIR)
    total = article.get_total_page(url.format(1))
    total = 4
    url_queue = Queue()
    ...
    for i in range(2):
        t = threading.Thread(target=article.run, args=(url_queue,))
        t.start()
